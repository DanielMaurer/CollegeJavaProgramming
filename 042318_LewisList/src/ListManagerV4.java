...
public class ListManagerV4 {
    ...
    /* the bonarySearch is an O(logbase2ofn) search algorithm.
     * Precondition: selectoinSort has already sorted the list.
     * This finction returns the posistion where the target was found, or 
     * -1 if its not found. 
     */
    public static int binarySearch(int target) {
    		...
    		hi = count - 1;
    		mid = (lo + hi)/2;
    		while(target != numbers[mid] && lo <= hi) {
    ...
    public static int recBinarySearch(int target, int lo, int hi) {
    		/*test for base cases
    		 * 1. you found the value at location mid (found it)
    		 * 2. lo > hi (didnt find it)
    		 */
    		if(lo > hi) {
    			return -1;
    		}
    		...
    		if (target == numbers[mid]) {
    			...
    			// decomposition - have function call itself
    			if(target > numbers[mid]) {
    				return recBinarySearch(target, mid + 1, hi); // right side of the list
    			} else {
    				return recBinarySearch(target, lo, mid - 1); // left part of list
    			}
    		}
    }
    ...
    public static void main(String[] args) {
        ...
        selectionSort();
        print();
        System.out.print("What do you want to search for using binary search: ");
        searchFor = sc.nextInt();
        foundAt = recBinarySearch(searchFor,0, count - 1);
        if(foundAt < 0) {
        		System.out.println("Not found.");
        } else {
            System.out.println("Found at " + foundAt);
