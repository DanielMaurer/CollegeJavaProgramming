...
class Song{
	...
	public Song(String title, int seconds, double frequency, int bDepth) { // constructor 1
		setName(title);
		setLength(seconds);
		setSamplingRate(frequency);
		setBitDepth(bDepth);
	}
	public Song(String title, int minutes, int seconds, double frequency, int bDepth) { // constructor 2
...
public class MaurerMusicV2 {
	...
	public static void main(String[] args) {
		...
		while(doAgain.equals("y")) {
			...
			bitDepth = sc.nextInt();
			// create song object
			mySong = new Song(songName, minutes, seconds, frequency, bitDepth);
